...
try {
    ...
    try {
      db_execute(
        "INSERT INTO seller_verification_requests (
            ...
            account_number, routing_number, branch_name, status, created_at
         ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', NOW())",
        [
          $user_id,
          $nid !== '' ? $nid : null,
  ...
  // For sellers, require admin approval before they can use the seller dashboard
  if ($role === 'seller') {
    header("Location: " . BASE_URL . "/html/login.php?info=seller_pending");
    exit;
  }

  // Auto-login after successful registration for non-seller roles
