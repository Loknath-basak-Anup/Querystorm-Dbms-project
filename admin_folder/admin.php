...
async function updateUserStatus(userId, status) {
            ...
            try {
                ...
                banners.forEach(b => {
                    ...
                    tr.innerHTML = `
                        ...
                        <td>${b.title || ''}</td>
                        <td>${b.subtitle || ''}</td>
                        <td><a href="${b.image_url || '#'}" target="_blank" rel="noopener">${b.image_url || ''}</a></td>
                        <td>${b.link_url ? `<a href="${b.link_url}" target="_blank" rel="noopener">${b.link_url}</a>` : ''}</td>
                        <td>${b.position || ''}</td>
                        <td><span class="badge ${b.is_active ? 'badge-success' : 'badge-danger'}">${b.is_active ? 'Active' : 'Inactive'}</span></td>
                        <td>${new Date(b.created_at).toLocaleString()}</td>
                        <td>
                            <button class="action-btn btn-edit" onclick='editBanner(${JSON.stringify(b)})'>Edit</button>
                            <button class="action-btn btn-approve" onclick="toggleBannerActive(${b.banner_id}, ${b.is_active ? 0 : 1})">${b.is_active ? 'Hide' : 'Show'}</button>
        ...
        if (bannerForm) {
            bannerForm.addEventListener('submit', async (e) => {
                ...
                const resp = await fetch('?ajax=save_banner', { method: 'POST', body: formData });
                const result = await resp.json();
                if (!result.success) {
                    alert(result.error || 'Could not save banner');
                    return;
                }
                resetBannerForm();
                loadBanners();
        ...
        function editBanner(banner) {
            document.getElementById('banner-id').value = banner.banner_id || '';
            document.getElementById('banner-title').value = banner.title || '';
            document.getElementById('banner-subtitle').value = banner.subtitle || '';
            document.getElementById('banner-position').value = banner.position || 'products_top';
            document.getElementById('banner-sort').value = banner.sort_order || 0;
            document.getElementById('banner-link').value = banner.link_url || '';
            document.getElementById('banner-starts').value = banner.starts_at ? banner.starts_at.replace(' ', 'T') : '';
            document.getElementById('banner-ends').value = banner.ends_at ? banner.ends_at.replace(' ', 'T') : '';
        ...
        function resetBannerForm() {
            ...
            document.getElementById('banner-position').value = 'products_top';
            document.getElementById('banner-sort').value = 0;
            document.getElementById('banner-link').value = '';
            document.getElementById('banner-starts').value = '';
            document.getElementById('banner-ends').value = '';
