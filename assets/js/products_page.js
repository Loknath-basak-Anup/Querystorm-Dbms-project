...
function isBuyerLoggedIn() {
    if (window.quickmartSession && typeof window.quickmartSession.loggedIn === 'boolean') {
        return window.quickmartSession.loggedIn && window.quickmartSession.role === 'buyer';
    }
...
function getCartPageUrl() {
    ...
    if (path.includes('/buyer_dashboard/')) return 'cart.php';
    if (path.includes('/html/')) return '../buyer_dashboard/cart.php';
    return 'buyer_dashboard/cart.php';
}
...
function attachEventListeners() {
    ...
    if (cartCheckoutBtn) {
        cartCheckoutBtn.addEventListener('click', () => {
            if (!isBuyerLoggedIn()) {
                if (typeof showGuestModal === 'function') {
                    showGuestModal();
                } else {
    ...
    productsGrid.innerHTML = productsToShow.map((product, index) => `
        <div class="product-card" data-aos="zoom-in-up" data-aos-delay="${index % 12 * 50}" data-product-id="${product.id}" style="cursor:pointer">
            <div class="product-image-container">
                ...
                ${!product.inStock ? '<div class="stock-overlay"></div><div class="stock-ribbon">Stock Out</div>' : ''}
...
function addToCart(productId) {
    if (isBuyerLoggedIn()) {
        ...
        fetch(getCartApiUrl(), {
            ...
            .then(data => {
                if (!data || !data.success) {
                    showNotification(data && data.message ? data.message : 'Could not add to cart.', 'error');
                    return;
                }
                updateCartCount();
                renderCartItems();
...
function updateQuantity(productId, change) {
    if (isBuyerLoggedIn()) {
        const nextQuantity = Math.max(0, parseInt(change, 10) || 0);
        const form = new URLSearchParams();
        form.append('action', 'update');
        form.append('product_id', String(productId));
        form.append('quantity', String(nextQuantity));
        ...
        fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            ...
            .then(data => {
                ...
                cartItems.innerHTML = items.map(item => {
                    const imageUrl = item.image_url ? normalizeImageUrl(item.image_url) : (window.placeholderImage || '');
                    return `
                        <div class="cart-item">
                            <img src="${imageUrl}" alt="${item.name}" class="cart-item-image">
                            <div class="cart-item-info">
                                <h4 class="cart-item-title">${item.name}</h4>
                                <p class="cart-item-price">${Number(item.price).toLocaleString()} BDT</p>
                ...
                cartTotal.textContent = `${Number(total).toLocaleString()} BDT`;
            })
            .catch(err => {
...
function showProductDetails(productId) {
    ...
    const coupons = (window.productCoupons && window.productCoupons[product.id])
        ? window.productCoupons[product.id]
        : [];
    ...
    modalBody.innerHTML = `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:2rem;margin-bottom:2rem">
            ...
            <div>
                ...
                <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:1rem">
                    <div id="reviewStars" style="color:var(--warning-color)">
        ...
        <div style="background:var(--bg-secondary);padding:1.5rem;border-radius:1rem;margin-bottom:2rem">
            ...
            <div style="display:grid;gap:1rem">
                ${coupons.length === 0 ? `
                    <div style="color:var(--text-muted);font-size:0.9rem;">No coupons approved for this product yet.</div>
                ` : coupons.map(coupon => `
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:1rem;background:var(--bg-primary);border-radius:0.5rem;border:2px dashed var(--primary-color)">
                        <div>
                            ...
                            <div style="font-size:0.875rem;color:var(--text-muted)">
                                ${coupon.discount_type === 'percent' ? `Get ${coupon.discount_value}% off` : `Save ${coupon.discount_value} BDT`}
                                ${coupon.min_purchase ? ` on orders above ${coupon.min_purchase} BDT` : ''}
                            </div>
                        </div>
        ...
        <!-- Reviews Section -->
        <div style="margin-bottom:2rem" data-product-id="${product.id}">
            ...
            <div id="reviewSummary" style="display:flex;align-items:center;gap:1rem;margin-bottom:1rem;color:var(--text-muted)">
                <div id="reviewAvg" style="font-weight:700;color:var(--text-primary)">0.0</div>
                <div id="reviewStarsInline" style="color:var(--warning-color)">${'<i class="far fa-star"></i>'.repeat(5)}</div>
                <div id="reviewTotal">(0 reviews)</div>
            </div>
            <div id="reviewFormWrap" style="padding:1.25rem;background:var(--bg-secondary);border-radius:1rem;margin-bottom:1.5rem"></div>
...
function applyCoupon(code) {
    playSound('coupon');
    try {
        localStorage.setItem('quickmart_coupon_code', code);
    } catch (e) {
        // ignore storage errors
    }
    showNotification(`Coupon ${code} saved. Apply it in your cart.`, 'success');
}

function getReviewsApiUrl() {
    const path = window.location.pathname;
    const lower = path.toLowerCase();
    const marker = '/quickmart/';
    const idx = lower.indexOf(marker);
    if (idx !== -1) {
        const base = path.substring(0, idx + marker.length);
        return base + 'actions/product_reviews.php';
    }
    if (path.includes('/buyer_dashboard/') || path.includes('/seller_dashboard/')) {
        return '../actions/product_reviews.php';
    }
    if (path.includes('/html/')) {
        return '../actions/product_reviews.php';
    }
...
function loadProductReviews(productId) {
    ...
    const starsInline = document.getElementById('reviewStarsInline');
    const topStars = document.getElementById('reviewStars');
    const topCount = document.getElementById('reviewCount');

    if (!listEl) return;
    listEl.innerHTML = '<p style="color:var(--text-muted)">Loading reviews...</p>';

    fetch(`${getReviewsApiUrl()}?product_id=${productId}`, {
        headers: { 'X-Requested-With': 'XMLHttpRequest' }
    })
        .then(res => res.json())
        .then(data => {
            if (!data || !data.success) {
                listEl.innerHTML = '<p style="color:var(--text-muted)">No reviews found.</p>';
                return;
            }

            const reviews = Array.isArray(data.reviews) ? data.reviews : [];
            const avg = typeof data.average === 'number' ? data.average : 0;
            const total = typeof data.count === 'number' ? data.count : reviews.length;

            if (avgEl) avgEl.textContent = avg.toFixed(1);
            if (totalEl) totalEl.textContent = `(${total} reviews)`;
            if (starsInline) starsInline.innerHTML = renderStarIcons(avg);
            if (topStars) topStars.innerHTML = renderStarIcons(avg);
            if (topCount) topCount.textContent = `(${total} reviews)`;

            if (reviews.length === 0) {
                listEl.innerHTML = '<p style="color:var(--text-muted)">No reviews yet. Be the first to review.</p>';
                return;
            }
            ...
            listEl.innerHTML = reviews.map(review => {
                ...
                const imageBlock = review.image_url ? `
                    <img src="${review.image_url}" alt="Review image" style="margin-top:0.75rem;width:120px;height:120px;object-fit:cover;border-radius:0.5rem;border:1px solid var(--border-color)">
                ` : '';
                return `
                    <div style="padding:1.5rem;background:var(--bg-secondary);border-radius:1rem;margin-bottom:1rem">
                        <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:0.75rem">
                            <div>
                                <div style="font-weight:600;margin-bottom:0.25rem">${review.buyer_name || 'Buyer'}</div>
                                <div style="color:var(--warning-color);margin-bottom:0.5rem">
                                    ${renderStarIcons(Number(review.rating || 0))}
                                </div>
                            </div>
                            <span style="color:var(--text-muted);font-size:0.875rem">${dateText}</span>
                        </div>
        ...
        .catch(err => {
            ...
            listEl.innerHTML = '<p style="color:var(--text-muted)">Could not load reviews.</p>';
        });
}
...
function renderReviewForm(productId) {
    ...
    if (!isBuyerLoggedIn()) {
        wrap.innerHTML = `
            <div style="display:flex;justify-content:space-between;align-items:center;gap:1rem">
                <div style="color:var(--text-muted)">Sign in as a buyer to write a review.</div>
                <a href="../html/login.php" class="btn-add-cart" style="padding:0.5rem 1rem">Sign In</a>
            </div>
        `;
        return;
    }
    ...
    wrap.innerHTML = `
        <form id="reviewForm" style="display:grid;gap:0.75rem">
            <div style="display:flex;align-items:center;gap:0.5rem">
                <div style="font-weight:600">Your Rating:</div>
                <div id="reviewStarInput" style="display:flex;gap:0.25rem;color:var(--warning-color)">
                    ${[1,2,3,4,5].map(v => `<button type="button" data-value="${v}" style="background:none;border:none;cursor:pointer;font-size:1.1rem;color:var(--text-muted)"><i class="far fa-star"></i></button>`).join('')}
                </div>
                <input type="hidden" name="rating" id="reviewRating" value="0">
            </div>
            <textarea name="comment" id="reviewComment" rows="3" placeholder="Write your review..." style="width:100%;padding:0.75rem;border-radius:0.75rem;border:1px solid var(--border-color);background:var(--bg-primary);color:var(--text-primary)"></textarea>
            <div style="display:flex;align-items:center;gap:1rem">
                <input type="file" name="image" id="reviewImage" accept="image/*" />
                <img id="reviewPreview" alt="Preview" style="display:none;width:60px;height:60px;border-radius:0.5rem;object-fit:cover;border:1px solid var(--border-color)">
            </div>
            <button type="submit" class="btn-add-cart" style="justify-self:start;padding:0.6rem 1.5rem">Post Review</button>
        </form>
    `;

    const ratingInput = document.getElementById('reviewRating');
    const starWrap = document.getElementById('reviewStarInput');
    ...
    if (starWrap && ratingInput) {
        starWrap.querySelectorAll('button[data-value]').forEach(btn => {
            btn.addEventListener('click', () => {
                const value = parseInt(btn.getAttribute('data-value'), 10) || 0;
                ratingInput.value = String(value);
                starWrap.querySelectorAll('button[data-value]').forEach(starBtn => {
                    const starVal = parseInt(starBtn.getAttribute('data-value'), 10) || 0;
                    const icon = starBtn.querySelector('i');
                    if (icon) {
                        icon.className = starVal <= value ? 'fas fa-star' : 'far fa-star';
                        starBtn.style.color = starVal <= value ? 'var(--warning-color)' : 'var(--text-muted)';
                    }
                });
            });
        });
    ...
    if (form) {
        form.addEventListener('submit', (e) => {
            ...
            if (!comment && !(imageInput.files && imageInput.files.length)) {
                showNotification('Please write a review or add an image.', 'error');
                return;
            }
            ...
            fetch(getReviewsApiUrl(), {
                ...
                .then(data => {
                    if (!data || !data.success) {
                        showNotification(data && data.message ? data.message : 'Could not submit review.', 'error');
                        return;
                    }
                    commentInput.value = '';
                    ratingInput.value = '0';
