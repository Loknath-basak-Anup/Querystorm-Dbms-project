...
<body class="dark-mode">
                ...
                <div style="background:linear-gradient(145deg, #111827 0%, #0b1020 50%, #111827 100%); color:#f8fafc; border-radius:18px; padding:28px 24px; width:100%; max-width:680px; box-shadow:0 18px 48px rgba(0,0,0,0.35); position:relative; border:1px solid rgba(255,255,255,0.05);">
                    ...
                    <form id="addProductForm" style="display:flex; flex-direction:column; gap:14px;">
                        <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:14px;">
                            ...
                            <div style="display:flex; flex-direction:column; gap:6px;">
                                <label for="productSubcategory" style="font-weight:600; color:#e2e8f0;">Category / Subcategory</label>
                                <select id="productSubcategory" name="productSubcategory" required style="width:100%; padding:10px 12px; border-radius:10px; border:2px solid rgba(255,255,255,0.1); background:rgba(255,255,255,0.05); color:#e2e8f0; font-family:'Poppins',sans-serif; font-size:0.95rem; transition:all 0.3s ease; cursor:pointer;">
    ...
    <script>
        ...
        function renderPreview() {
            ...
            const selectedCategory = selectedOption ? (selectedOption.getAttribute('data-category-name') || '-') : '-';
            const selectedSubcategory = selectedOption ? (selectedOption.getAttribute('data-subcategory-name') || '-') : '-';
        ...
        if (addProductForm) {
            addProductForm.onsubmit = async function(e) {
                ...
                const selectedSub = subcategorySelect ? subcategorySelect.options[subcategorySelect.selectedIndex] : null;
                payload.append('category_id', selectedSub ? (selectedSub.getAttribute('data-category') || '') : '');
