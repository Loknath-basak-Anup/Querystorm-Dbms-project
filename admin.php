...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_GET['ajax'])) {
    ...
    if ($entered_password === $admin_password) {
        ...
        header('Location: admin.php');
        echo "<!DOCTYPE html><html><head><meta http-equiv='refresh' content='0;url=admin.php'></head><body><script>location.replace('admin.php');</script></body></html>";
        exit;
    } else {
...
if (isset($_GET['ajax'])) {
    ...
    if (!isset($_SESSION['admin_logged_in']) || $_SESSION['admin_logged_in'] !== true) {
        ...
        echo json_encode(['error' => 'Not authenticated', 'redirect' => 'admin.php']);
        exit;
    }
    ...
    switch ($_GET['ajax']) {
        case 'get_stats':
            $stats = [
                ...
                'pending_sellers' => db_fetch("SELECT COUNT(*) as count FROM seller_profiles WHERE verified = 0")['count'],
                ...
                'total_revenue' => db_fetch("SELECT COALESCE(SUM(total_amount), 0) as total FROM orders WHERE status = 'completed'")['total'],
            ];
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .category-toggle-btn { background: transparent; border: none; color: var(--muted); cursor: pointer; padding: 4px; }
        .category-toggle-btn i { font-size: 12px; }
        .inline-subcategory-table { width: 100%; border-collapse: collapse; font-size: 12px; margin-top: 6px; }
        .inline-subcategory-table th, .inline-subcategory-table td { padding: 6px 8px; border-bottom: 1px solid rgba(148, 163, 184, 0.18); }
        .inline-subcategory-table th { text-align: left; color: var(--muted); text-transform: uppercase; letter-spacing: 0.5px; font-size: 11px; }
...
<body>
                    ...
                    <div class="card-block">
                        ...
                        <div id="users-content" style="display:none;">
                            <div class="table-wrap">
                                <table id="users-table"><thead><tr><th>ID</th><th>Name</th><th>Email</th><th>Role</th><th>Status</th><th>Joined</th><th>Details</th><th>Actions</th></tr></thead><tbody id="users-tbody"></tbody></table>
                            </div>
                    ...
                    <div class="card-block">
                        <div class="card-header">
                            ...
                            <div style="display:flex; gap:10px;">
                                ...
                                <button class="btn-secondary" onclick="showAddSubcategoryModal()"><i class="fa-solid fa-layer-group"></i> Add Subcategory</button>
                            </div>
                        </div>
                        <div class="form-row" style="margin-bottom:16px;">
                            <div class="form-group" style="flex:2;">
                                <input id="category-search" type="text" placeholder="Search categories or subcategories">
                            </div>
                        ...
                        <div style="margin-top:24px;">
                            <div class="card-header" style="margin-bottom:12px;">
                                <h3 style="font-size:1.05rem; color:var(--muted);">Subcategories</h3>
                            </div>
                            <div class="loading" id="subcategories-loading"><div class="spinner"></div><p>Loading subcategories...</p></div>
                            <div id="subcategories-content" style="display:none;">
                                <div class="table-wrap">
                                    <table id="subcategories-table">
                                        <thead>
                                            <tr><th></th><th>Icon</th><th>Name</th><th>Type</th><th>Date</th><th>Actions</th></tr>
                                        </thead>
    ...
    <div id="add-subcategory-modal" class="modal">
        <div class="modal-content">
            <div class="modal-header"><h2>Add Subcategory</h2><button class="close-modal" onclick="closeModal('add-subcategory-modal')">&times;</button></div>
            <form id="add-subcategory-form">
                <div class="form-group"><label>Category</label><select id="subcategory-category" required></select></div>
                <div class="form-group"><label>Subcategory Name</label><input type="text" id="subcategory-name" required placeholder="e.g., Laptops"></div>
                <div class="form-group"><label>Icon Class</label><input type="text" id="subcategory-icon" placeholder="fa-solid fa-diagram-next"></div>
                <div class="form-group" style="display:flex; gap:10px; align-items:center;">
                    <button type="button" class="btn-secondary" onclick="previewIcon('add-subcategory-icon-preview', document.getElementById('subcategory-icon').value)">Preview Icon</button>
                    <div id="add-subcategory-icon-preview" style="width:32px; height:32px; border-radius:8px; border:1px solid rgba(148,163,184,0.4); display:flex; align-items:center; justify-content:center; color:var(--muted); font-size:12px;">N/A</div>
                </div>
                <button type="submit" class="btn-primary"><i class="fa-solid fa-layer-group"></i> Add Subcategory</button>
            </form>
        </div>
    </div>
    ...
    <div id="edit-subcategory-modal" class="modal">
        <div class="modal-content">
            <div class="modal-header"><h2>Edit Subcategory</h2><button class="close-modal" onclick="closeModal('edit-subcategory-modal')">&times;</button></div>
            <form id="edit-subcategory-form">
                <input type="hidden" id="edit-subcategory-id">
                <div class="form-group"><label>Category</label><select id="edit-subcategory-category" required></select></div>
                <div class="form-group"><label>Subcategory Name</label><input type="text" id="edit-subcategory-name" required></div>
                <div class="form-group"><label>Icon Class</label><input type="text" id="edit-subcategory-icon" placeholder="fa-solid fa-diagram-next"></div>
                <div class="form-group" style="display:flex; gap:10px; align-items:center;">
                    <button type="button" class="btn-secondary" onclick="previewIcon('edit-subcategory-icon-preview', document.getElementById('edit-subcategory-icon').value)">Preview Icon</button>
                    <div id="edit-subcategory-icon-preview" style="width:32px; height:32px; border-radius:8px; border:1px solid rgba(148,163,184,0.4); display:flex; align-items:center; justify-content:center; color:var(--muted); font-size:12px;">N/A</div>
                </div>
                <button type="submit" class="btn-primary"><i class="fa-solid fa-floppy-disk"></i> Save</button>
            </form>
        </div>
    ...
    <script>
        ...
        async function loadProducts() {
            try {
                const response = await fetch('?ajax=get_products', { credentials: 'same-origin' });
            ...
            try {
                ...
                categories.forEach(category => {
                    ...
                    tr.innerHTML = `
                        ...
                        <td>${category.name}</td>
                        <td>
                            <button class="category-toggle-btn" type="button" title="Show subcategories"><i class="fa-solid fa-chevron-right"></i></button>
                ...
                syncSubcategoryCategoryOptions();
        ...
        async function loadSubcategories() {
            try {
                ...
                subcategoriesCache.forEach(sub => {
                    const tr = document.createElement('tr');
                    tr.setAttribute('data-subcategory-id', sub.subcategory_id);
                    tr.setAttribute('data-category-id', sub.category_id);
                    tr.setAttribute('data-name', (sub.name || '').toLowerCase());
                    tr.setAttribute('data-category-name', (sub.category_name || '').toLowerCase());
                    const createdLabel = sub.created_at ? new Date(sub.created_at).toLocaleDateString() : '';
                    tr.innerHTML = `
                        <td><i class="fa-solid fa-angles-right" style="color: var(--muted);"></i></td>
                        <td>${renderIconCell(sub.icon_class, sub.icon_url)}</td>
                        <td>${sub.name}</td>
                        <td>${sub.category_name}</td>
                        <td>${createdLabel}</td>
                        <td>
                            <button class="action-btn" style="padding:4px 8px;" onclick="openAddSubcategoryForCategory(${sub.category_id})"><i class="fa-solid fa-plus"></i></button>
                            <button class="action-btn btn-edit" style="padding:4px 8px;" onclick="openSubcategoryEdit(${sub.subcategory_id})"><i class="fa-solid fa-pen"></i></button>
                            <button class="action-btn btn-delete" style="padding:4px 8px;" onclick="deleteSubcategory(${sub.subcategory_id})"><i class="fa-solid fa-trash"></i></button>
                        </td>
                    `;
                    tbody.appendChild(tr);
                });
                ...
                document.getElementById('subcategories-content').style.display = 'block';
                populateSubcategoryFilter(subcategoriesCache);
                applyCategoryFilters();
        ...
        function syncSubcategoryCategoryOptions() {
        ...
        function buildInlineSubcategoryHTML(categoryId) {
            ...
            if (items.length === 0) {
                return `<div style="color:var(--muted); font-size:12px; padding:6px 0; display:flex; align-items:center; gap:8px;">
                            No subcategories yet.
                            <button class="action-btn" style="padding:4px 8px;" onclick="openAddSubcategoryForCategory(${categoryId})"><i class=\"fa-solid fa-plus\"></i> Add</button>
                        </div>`;
            }
            ...
            return `
                <table class="inline-subcategory-table">
                    <thead>
                        <tr><th>Icon</th><th>Name</th><th>Type</th><th>Date</th><th>Actions</th></tr>
                    </thead>
                    <tbody>
                        ${rows}
                    </tbody>
        ...
        function openAddSubcategoryForCategory(categoryId) {
        ...
        function populateEditSubcategoryCategoryOptions(selectedId) {
            const select = document.getElementById('edit-subcategory-category');
            if (!select) return;
            select.innerHTML = '';
            categoriesCache.forEach(cat => {
                const option = document.createElement('option');
                option.value = cat.category_id;
                option.textContent = cat.name;
                if (String(cat.category_id) === String(selectedId)) {
        ...
        function openSubcategoryEdit(subcategoryId) {
            const sub = subcategoriesCache.find(s => String(s.subcategory_id) === String(subcategoryId));
            if (!sub) return;
            document.getElementById('edit-subcategory-id').value = sub.subcategory_id;
            document.getElementById('edit-subcategory-name').value = sub.name || '';
            document.getElementById('edit-subcategory-icon').value = sub.icon_class || '';
            populateEditSubcategoryCategoryOptions(sub.category_id);
            previewIcon('edit-subcategory-icon-preview', sub.icon_class || '');
            document.getElementById('edit-subcategory-modal').classList.add('show');
        }

        function populateSubcategoryFilter(items) {
        ...
        function applyCategoryFilters() {
            ...
            const selectedSub = (filterSelect && filterSelect.value) ? filterSelect.value : '';

            const categoryRows = document.querySelectorAll('#categories-tbody tr.category-row');
            const subcategoryRows = document.querySelectorAll('#subcategories-tbody tr');
            ...
            subcategoryRows.forEach(row => {
                ...
                const matchesSearch = term === '' || name.includes(term) || catName.includes(term);
                const matchesSelect = selectedSub === '' || subId === selectedSub;
                const show = matchesSearch && matchesSelect;
                row.style.display = show ? '' : 'none';
                if (show) {
                    visibleCategoryIds.add(row.getAttribute('data-category-id') || '');
                }
            });
            ...
            categoryRows.forEach(row => {
                ...
                const matchesSubFilter = selectedSub === '' || visibleCategoryIds.has(catId);
                row.style.display = (matchesSearch && matchesSubFilter) ? '' : 'none';
            });

            // Hide inline detail rows when their parent category is hidden
            const detailRows = document.querySelectorAll('#categories-tbody tr.category-subcats-row');
            detailRows.forEach(row => {
                const catId = row.getAttribute('data-category-id') || '';
                const parent = document.querySelector(`#categories-tbody tr.category-row[data-category-id="${catId}"]`);
        ...
        async function deleteSubcategory(subcategoryId) {
            if (!confirm('Delete this subcategory? Products in this subcategory will be affected.')) return;
            try {
                const formData = new FormData();
        ...
        async function showAddSubcategoryModal() {
            ...
            syncSubcategoryCategoryOptions();
            document.getElementById('add-subcategory-modal').classList.add('show');
        }
        ...
        if (subcategoryFilter) {
            subcategoryFilter.addEventListener('change', applyCategoryFilters);
        }
        ...
        document.getElementById('add-subcategory-form').addEventListener('submit', async (e) => {
            ...
            try {
                const formData = new FormData();
                formData.append('category_id', document.getElementById('subcategory-category').value);
                formData.append('name', document.getElementById('subcategory-name').value.trim());
                formData.append('icon_class', normalizeIconClass(document.getElementById('subcategory-icon').value));
        ...
        document.getElementById('edit-subcategory-form').addEventListener('submit', async (e) => {
            e.preventDefault();
            try {
                const formData = new FormData();
                formData.append('subcategory_id', document.getElementById('edit-subcategory-id').value);
                formData.append('category_id', document.getElementById('edit-subcategory-category').value);
                formData.append('name', document.getElementById('edit-subcategory-name').value.trim());
                formData.append('icon_class', normalizeIconClass(document.getElementById('edit-subcategory-icon').value));
                const response = await fetch('?ajax=update_subcategory', {
                    method: 'POST',
                    body: formData
                });
                const result = await response.json();
                if (result.success) {
                    ...
                    alert('Error: ' + (result.error || 'Failed to update subcategory'));
                }
            } catch (error) {
        ...
        // Delegate clicks on category arrow buttons so they always work
